...
- **`XIT/FTC`**：面板并列展示服务器当前对该航线下发的原生飞行计划总时长 —— 由各段 `(arrival - departure)` 累加，与 FTC 建议的 f 无关（按当前飞船 f/载重，独立于最优方案）；缺值（计划未下发）时不显示。邻档对比表新增「最省」/「最快」标签（同一行可同时满足两类，如极端支配点）。

### 🔧 Improvements

- **`XIT/FTC`**：系内 STL 转移段速度改用 BTF 直采实测常数 27,512 km/s —— 与 f、距离无关（替换原 87,036×(f/0.5)^0.84 Weibull 拟合式在 500M km 量级偏快 ~2×），时长 = `d / (27,512 × 3600) / cond`；面板与 SFC 自动联动两边口径一致。代价：平衡退化（`tSpan = 0`）→ 最省油即 `f = 0.05`。
- **`XIT/FTC`**：`findNativeFlightPlan` 查询下沉到 `computeFtcPlan`（`out.nativePlan`），FTC.vue 只消费结果 —— 避免两处重复调用并确保「航线明细」与「实测总时长」来自同一份原生计划；查表统一用反查后的实体键（`metrics.fromLookup/toLookup`），SFC 把空间站目的地规范化成所属星系 id 的命不中问题同时修复。
...
- **`XIT/FTC`**：系内（同一星系）航线改用服务器原生飞行计划里的「转移」（TRANSIT）段记录与 BTF 实测标定口径自动设定档位 —— 转移段燃料与距离、载重无关，且 f ≥ 0.5 饱和；时长按同一批实测点拟合，仍有接近一成的偏差。此前系内航线拿不到几何，一律不自动设定。

### 🔧 Improvements

- **`XIT/FTC`**：STL 起降/转移几何一律取服务器下发的原生飞行计划段记录，不再用自建轨道模型与统计常数估算、也不再靠浏览星系补几何 —— 同一条航线在面板与 SFC 自动联动下结果一致（此前两边会给出不同档位）。代价：服务器尚未下发该航线记录时不给出估算值（面板提示「输入不完整」，SFC 联动沿用玩家当前设置）；系内转移段里程会随计划时刻（行星相位）小幅变化，不是航线常数。
- **`XIT/FTC`**：最优档位改按「航线」保存（飞船 + 起终点 + 是否走跃迁点）—— 飞船换目的地后不再沿用上一条航线的档位。旧版按飞船保存的档位一次性作废：升级后每条航线需先算出一次完整结果才会恢复自动设定，期间保持玩家当前设置（不会写入错值）。
- **`SFC`**：自动联动加推送门 —— 同一条航线、同一批几何数据最多计算一次，目的地输入框打字停顿、表格重渲染、拖滑块等界面抖动不再反复触发整轮燃料档位扫描（未取到公司蓝图时还含最长 6 秒的等待），也不再出现「写档位 → 服务器重算 → 再写」的循环；几何数据晚到时仍会补算，没有次数上限。
- **`XIT/FTC`**：「输入不完整」提示改为写清成因与可执行动作（在 SFC 里为该航线生成一次飞行计划并等服务器算完 / 把目的地改成具体天体或空间站），且只列真正成立的成因；首推时因时序造成的残缺降为调试日志。

### 🐞 Fixes

- **`SFC`**：修复自动设定把燃料档位写成 `1`（滑块拉满）—— 航线几何或罐容量等必需输入残缺时改为不写档位、沿用玩家当前设置；燃料或时间在所有候选方案中无差别时改取最省油方案，不再返回最快档，也不再用浮点微差放大成全量程。
- **`XIT/FTC`**：目的地是空间站时（SFC 会把空间站规范化成所属星系 id）按唯一命中的空间站反查后再查服务器记录 —— 此前查表键是星系 id，必然命不中，该航线永远算不出结果；候选不唯一或无空间站数据时仍拒绝反查（宁可不写也不猜）。
...
- **`XIT/CQ`**：他人挂单查询 —— 按公司代码/名称/用户名（输入框或 `XIT CQ <参数>`），经 FIO 公开 API 一次拉取该公司在全部 CX 交易所的挂单与本地市场（LM）广告；CX 表可一键打开对应 `CXOB {ticker}.{exchange}` 订单簿，LM 商品广告行可一键打开 `LMA {planet}/{ad}`，LM 运输广告行打开 `LM {planet}`；材料列同时显示中文名与 ticker 代码。FIO 数据为社区上传的快照，非游戏实时全量。

### 🐞 Fixes

- **`XIT BURN`**：消耗舍入到 `-0.00` 的天数从 `0` 改为 `∞`（之前库存为 0 时 `0 / 0.001 = 0`）。
...
- **`XIT/BMAT`**：建材购买表 —— 从 JH 已保存计划聚合建材需求，复用 BPC 比价引擎（AI1/CI1/IC1/NC1 四玩家交易所 CXOB 实时盘）输出最优混合采购 ACT 包；计划行「购买表」一键直达。
- **`XIT/CBUY`**：购物车购买表 —— 直接由购物车生成多交易所比价购买包，购物车 ActionBar 一键直达。

### 🔧 Improvements

- **`XIT/FLEET`**：环线多端同步加固（冲突闸 + 代际指纹）——
  - 冲突闸：409/LWW 拒绝后自动推送跳过该条目，须在「云端同步」对话框手动下拉/强制上传，避免时钟偏差下 409 刷新基准后静默覆盖他端更新；跨会话持久化。
  - 代际指纹：execute 每船铸入 10 位 hex fp 并嵌入 ACT 包/触发器名末尾，脚本归属/清理按 fp（船改名、名字互为后缀免疫）；云端快照带 fp/clearedFp 轻量 tombstone，防旧数据复活。
  - 409 代际规则与补推：云端清了本地同一代 → 本地自动采纳删除；本地清了同一代而云端仍是同代 run → 显式清理赢过自动进度；过期清空意图被新代际取代时静默收敛；启动时补推未落地的清理。
  - 包名长度保护（保留数字/类型前缀与末尾 fp）；展示层主包定位改 `findRunMainPkg`（mainPkgName → fp → 古早回退），取代按船名切片反推。
...
- **`XIT/FLEET`**：时间均衡分段时长口径与瓶颈容量诊断 —— 时间均衡横幅改为与各船卡片「飞行时长」同口径（优先采用切段后的精确实测，DP 估算仅在精确实测未就绪时作过渡），瓶颈即最慢段；瓶颈站无法转给相邻非空段船分担时，以真实吨位/体积说明原因（如「WCB 接不下 CCD：该站约 Xt / Ym³，其剩余约 …t / …m³」），仅在确实因剩余舱容装不下时才提示，时间最优的结果不误报。
...
- **`XIT/FLEET`**：环线云端同步防静默覆盖与冲突修复 —— 同步快照与脚本原先“任一变化就全量推送”且会反复推进云端时间戳，可能覆盖他端对 config/闲置船的更新（架空 LWW）；现推送前以内容签名（剔除 `updatedAt` 与触发器本地 `id`）比对，未实际变化即跳过且不推进时间戳；同步基准（CAS base / LWW 时间 / 推送签名）持久化到 localStorage（按公司隔离），页面重载后恢复 CAS/LWW/签名语义，陈旧本地不再静默覆盖云端；推送改为串行执行，同步对话框“上传/覆盖”在自动推送在途时排队而非被静默丢弃；对话框等待在途推送结束后再比较；“下拉覆盖”（confirmPull）后刷新乐观锁/LWW 基准、以“落地后”本地内容记录签名，避免假 409 与拉取后冗余推送。
- **`XIT/FLEET`**：环线脚本归属改为 shipId 级关联 —— 该船环线 ACT 包/触发器原按“当前净化船名”后缀匹配：船改名后找不到旧名包（推送变空、误删云端或留下孤儿），船名互为后缀（如 Cargo / Big Cargo）会互相误收误删；现以 `chainRuns[shipId]` 固化的包名（main/final/stops，执行时写入、与后续改名无关）为权威来源，当前船名仅作兜底且已被其它船运行时让出；`collectShipDoc`/`applyShipDoc`/清理（`removeShipChainScripts`）统一使用该归属判定，`applyShipDoc` 的删除范围以覆盖 `chainRuns` 前的本地归属快照为准。
- **`XIT/FLEET`**：手动“清理计划”后删除未传播到云端 —— 清空本地该船运行记录与脚本后，该船已不在同步 watch 覆盖范围内，云端保留陈旧副本；清理确认后显式标脏该船并把“空快照”推上云端，清除远端陈旧记录（他端仍在更新时由 LWW/CAS 保守拒绝）。

### 🔧 Improvements

- **`XIT/FLEET`**：同步基准写回与提示细节 —— 同步基准持久化改为合并写（先读现有再覆盖本实例条目，多标签页/多实例不再互相清除）且仅在基准实际变化时写；冲突状态在无待推本地差异时自动清除（避免“有冲突”残留）；同步提示横幅数秒后自动消失，不再常驻遮挡。
