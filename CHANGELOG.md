...
- **`XIT/SELL`**：新增批量售卖面板 —— 物品代码逗号分隔输入、仓库框选拖入或 JSON 识别导入（每项可覆盖交易所/排名/数量），读取四大交易所（IC1/NC1/AI1/CI1）CX 仓库库存；支持「挂单售卖」（按排名压价，默认卖价第一名）与「填单售卖」（按买一价立即成交）两种模式，实时显示自动价格与预计入账，一键生成 CX Sell 操作包（执行成功后自动删除）。
- **`XIT/ACT`**：新增 CX Sell 动作与 CXPO_SELL 步骤 —— 静默打开 CXPO 面板挂单/填单售卖；挂单按「排除自己后压过第 rank 名卖价一档」定价（`getSellLimitPrice`），填单按买一价下单并等待成交；支持并行组并发售卖与「跳过不足材料」。
- **`XIT/FLEET`**：新增产业链环线模式 —— 由各基地产出/消耗/库存推断上下游链关系并拓扑排序定航线（循环依赖断链并警告），按目标天数平衡链上物资运量（多上游按库存比例分摊、缺口回落 CX 采购），沿航线模拟舱容、超载按比例缩减提取量，无链关系基地的产物作为最终产物运回出发地；一键生成主包（加油+采购+装船+飞首站）与各站「卸货→提取→飞下一站」操作包 + 到港一次性触发器 + 归航包。
