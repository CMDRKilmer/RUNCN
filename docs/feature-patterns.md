...
- **时间/成本口径修正（2026-09-24，用户实测「FTC 结果与服务器 SFC 不一致」）**：同一份 16 段原生计划（`AVI-06JVV ZV-194h → vh-331` 勾网关：STL 起降 221.1562M km、FTL 50、STL 503、**总时长 20h16m40s、网关费 12,000 ICA**）暴露四处口径错误，四处均已修：
  ...
  - **网关锁定/衰变 = 各 10 秒（20 秒/段），旧值 20 分钟/段是单位错误**；**网关费 = 6,000 ICA/段**（旧实现硬编码 `gatewayCost = 0`，`totalCost` 漏掉这 12,000；它不随 f 变，故不影响最优 f）。常数唯一定义在 `fuel-model.ts`（`GW_PC_PER_H` / `GW_LOCK_HOURS` / `GW_COST_PER_JUMP`，route-planner 反向导入，避免两份副本再次漂移）。
  ...
  - 回归：`scripts/verify-ftc-gateway-stl.mjs` ⑨~⑬（三段分配逐位值 + 纯自然/系内等价性 + 残缺不归零、充能次数 = 跳数−1、网关时长 = `gwPc/3 + gwCount×20/3600`、网关费 6,000/段计入 `totalCost`、反查键匹配 16 段原生计划且输入原文命不中）；`scripts/verify-ftc-balance-degenerate.mjs` 场景 i 的 `totalHours` 由 4.6667 改为 4.0111（旧断言锁的是错常数）。
