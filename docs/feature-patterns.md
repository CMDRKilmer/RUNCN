...
- **⚠️ 上一条（2026-09-23 第三轮）的「口径未标定 → 仍判缺、不写滑块」「燃料与时长口径都未标定」、其「交叉验证 ② 时长」段落（模型 v(f) 恒高于实测平均速度、时长快 1.02–1.43×）以及旧「模型 2395 s vs 原生 2639 s」的对比，均已被 2026-09-23 收尾的 BTF 标定取代**：燃料口径 = 罐口径、时长口径 = 转移段速度式（数字见下方「系内「转移」段几何 + 燃料口径标定 + 星系 id 反查」条目）；`transitCalibrationPending` 门已删除，`missingModelInputs` 对系内航线**只剩两条**判缺理由（「口径未标定」这一条确实已删除）：① 原生转移段记录还没到（`fuel-model.ts` 第 938 行 `const dMissing = metrics.stlDistanceKm === undefined || !(metrics.stlDistanceKm > 0);`）；② STL 罐容量/余量缺失（同文件第 1048 行 `const tank = ship.stlRemaining ?? ship.stlFuelCapacity;`，为 undefined 或 ≤0 即判缺）。同一条里的「0.05% 比法」已作废（见下条），但**「系内航线的路程与是否勾选「使用跃迁点」无关」这一结论成立** —— 判据是 `planRoutes()` 对同星系两种模式都返回 `natural`（`legs` 为空），不是那两个数。
