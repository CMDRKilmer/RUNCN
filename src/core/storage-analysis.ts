...
// 物品尺寸使精确 100% 填充罕见且溢出不可能 — 将超过此值视为已满。
const STORAGE_FULL_THRESHOLD = 0.99;

// 仅在实际接近填满时才触发黄色告警 — 否则每个缓慢填满的基地都会提前数日标红，远未达可行动阈值。
...
// XIT BS 的 Inv 列告警：仓储（接近）满时为红色；距离填满 ≤ YELLOW_DAYS_THRESHOLD 天
// 且在该基地下次预期补给（最紧迫消耗物料烧至 1 天剩余的时刻）前即将填满时为黄色。
// 飞往该基地的飞船以其全部货舱容量计入额外仓储空间，因为它到达后会运走产出货物 ——
// 这既能防止告警，又能在飞船派遣后清除告警。
export function getStorageAlarmLevel(
  ...
  if (fillWeight >= STORAGE_FULL_THRESHOLD || fillVolume >= STORAGE_FULL_THRESHOLD) {
    ...
    return { level: 'red', reason: `仓储已满 (${binding})` };
  }
  ...
  if (daysUntilFull <= YELLOW_DAYS_THRESHOLD && daysUntilFull < nextResupplyDays) {
    return {
      level: 'yellow',
      reason: `将在 ${formatDaysShort(daysUntilFull)} 后填满，早于下次补给 (${formatDaysShort(nextResupplyDays)})`,
      days: daysUntilFull,
    };
  }
  return { level: 'none' };
...
// 返回表示基地 STORE 完成一次补给轮换后的合成 Store：净产出（生产）物料已运走，
// 消耗物料已补足至 computeNeed 数量。容量不变。由 CargoBar 用于可视化预计填充。
export function buildProjectedStore(
  ...
  // 严格净产出的物料 ticker —— 其现有库存假定在轮换中已运走。零日均物料（闲置库存）保留。
