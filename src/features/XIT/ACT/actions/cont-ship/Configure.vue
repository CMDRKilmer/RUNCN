...
function displayValue(value: string | undefined) {
  ...
  if (value.startsWith(groupTargetPrefix)) {
    return `[${value.slice(groupTargetPrefix.length)}] ${getI18nValue('RP.ACT.target', 'target')}`;
  }
  return value;
}
</script>
...
<template>
  <form>
    <Active
      v-if="data.contOrigin === configurableValue"
      :label="getI18nValue('RP.ACT.fields.from', 'From')">
      <SelectInput v-model="config.origin" :options="locations" />
    </Active>
    <Passive v-else :label="getI18nValue('RP.ACT.fields.from', 'From')">
      <span>{{ displayValue(data.contOrigin) }}</span>
    </Passive>
    <Active
      v-if="data.contDest === configurableValue"
      :label="getI18nValue('RP.ACT.fields.to', 'To')">
      <SelectInput v-model="config.destination" :options="locations" />
    </Active>
    <Passive v-else :label="getI18nValue('RP.ACT.fields.to', 'To')">
      <span>{{ displayValue(data.contDest) }}</span>
    </Passive>
    <Active
      v-if="data.autoProvision"
      :label="getI18nValue('RP.ACT.actions.contShip.autoProvision', 'Auto-provision')">
