...
const groupTargetOptions = computed(() =>
  pkg.groups
    .filter(x => x.name)
    .map(x => ({
      label: `[${x.name}] ${getI18nValue('RP.ACT.target', 'target')}`,
...
<template>
  <Active :label="getI18nValue('RP.ACT.fields.materialGroup', 'Material Group')">
    <SelectInput v-model="materialGroup" :options="materialGroups" />
  </Active>

  <Active :label="getI18nValue('RP.ACT.fields.origin', 'Origin')">
    <SelectInput v-model="contOrigin" :options="locationOptions" />
  </Active>

  <Active :label="getI18nValue('RP.ACT.fields.destination', 'Destination')">
    <SelectInput v-model="contDest" :options="locationOptions" />
  </Active>

  <Active :label="getI18nValue('RP.ACT.fields.currency', 'Currency')">
    <SelectInput v-model="currency" :options="currencies" />
  </Active>

  <Active :label="getI18nValue('RP.ACT.actions.contShip.paymentPerTon', 'Payment per Ton')">
    <NumericInput v-model="paymentPerTon" :min="0" :step="1" placeholder="0" />
  </Active>

  <Active :label="getI18nValue('RP.ACT.actions.contShip.daysToFulfill', 'Days to Fulfill')">
    <NumericInput v-model="daysToFulfill" :min="1" :max="30" :step="1" />
  </Active>

  <Active :label="getI18nValue('RP.ACT.actions.contShip.autoProvision', 'Auto-provision')">
    <RadioItem v-model="autoProvision">{{
      getI18nValue('RP.ACT.actions.contShip.enableAuto', 'enable auto-provision')
    }}</RadioItem>
  </Active>
</template>
