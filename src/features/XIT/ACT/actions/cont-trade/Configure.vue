...
<template>
  <form>
    <Active
      v-if="data.contLocation === configurableValue"
      :label="getI18nValue('RP.ACT.fields.location', 'Location')">
      <SelectInput v-model="config.location" :options="locations" />
    </Active>
    <Passive v-else :label="getI18nValue('RP.ACT.fields.location', 'Location')">
