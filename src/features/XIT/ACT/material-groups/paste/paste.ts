...
act.addMaterialGroup<Config>({
  type: 'Paste',
  shortDescription: getI18nValue(
    'RP.ACT.materialGroup.paste.short',
    'Paste materials from clipboard at execution time',
  ),
  description: () => {
    return getI18nValue('RP.ACT.materialGroup.paste.desc', 'Paste materials at execution time');
  },
  editComponent: Edit,
  ...
  generateMaterialBill: async ({ config, log, setPrices }) => {
    ...
    if (!result) {
      log.error(
        getI18nValue('RP.ACT.materialGroup.paste.invalid', 'Invalid or missing pasted materials.'),
      );
      return undefined;
    }
    if (result.prices) {
