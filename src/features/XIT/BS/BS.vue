      ...
      <thead>
        <tr>
          <th :class="[$style.narrowCol, $style.sortable]" @click="setSort('name')">
            {{ getI18nValue('RP.BS.planet', 'Planet') }}
            <span :class="isSorted('name') ? $style.sortActive : $style.sortInactive">{{
              getSortIndicator('name')
            }}</span>
          </th>
          <th v-if="showCmds" :class="[$style.narrowCol, $style.centered]">{{
            getI18nValue('RP.BS.cmd', 'CMD')
          }}</th>
          <th
            v-if="showBurn"
            :class="[$style.narrowCol, $style.sortable, $style.centered]"
            @click="setSort('burn')">
            {{ getI18nValue('RP.BS.burn', 'Burn') }}
            <span :class="isSorted('burn') ? $style.sortActive : $style.sortInactive">{{
              getSortIndicator('burn')
            }}</span>
          </th>
          <th v-if="showProd" :class="[$style.narrowCol, $style.centered]">{{
            getI18nValue('RP.BS.prod', 'Prod')
          }}</th>
          <th
            v-if="showRepair"
            :class="[$style.narrowCol, $style.sortable, $style.centered]"
            @click="setSort('repair')">
            {{ getI18nValue('RP.BS.repair', 'Rep') }}
            <span :class="isSorted('repair') ? $style.sortActive : $style.sortInactive">{{
              getSortIndicator('repair')
            }}</span>
          </th>
          <th v-if="showInv" :class="$style.invCol">{{ getI18nValue('RP.BS.inv', 'Inv') }}</th>
          <th v-if="showWar" :class="$style.warCol">{{ getI18nValue('RP.BS.war', 'War') }}</th>
        </tr>
      </thead>
      <tbody>
        <BaseRow
