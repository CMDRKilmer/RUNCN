...
const provenanceLabel = computed(() => {
  switch (source.value?.provenance) {
    case 'prun-live':
      return getI18nValue('RP.DATA.provenance.live', 'PRUN LIVE');
    case 'prun-live-with-defaults':
      return getI18nValue('RP.DATA.provenance.defaults', 'PRUN LIVE + BUNDLED DEFAULTS');
    case 'fio-reference-with-prun-overrides':
      return getI18nValue('RP.DATA.provenance.fio', 'FIO REFERENCE + PRUN OVERRIDES');
    default:
      return '';
  }
});
...
function loadSource() {
  ...
  if (!requestAvailable.value) {
    loadMessage.value = getI18nValue(
      'RP.DATA.transportUnavailable',
  ...
  try {
    ...
    loadMessage.value = getI18nValue('RP.DATA.loadRequested', 'Load requested from PrUn.');
  } catch (error) {
    loadMessage.value = getErrorMessage(error);
  }
}
...
<template>
  <div :class="$style.root">
    <div :class="$style.controls">
      <div :class="$style.settingsGrid">
        <section :class="$style.settingsPanel">
          <SectionHeader>{{ getI18nValue('RP.DATA.dataset', 'Dataset') }}</SectionHeader>
          <form>
            <Active :label="getI18nValue('RP.DATA.source', 'Source')">
              <SelectInput
        ...
        <section :class="$style.settingsPanel">
          <SectionHeader>{{ getI18nValue('RP.DATA.query', 'Query') }}</SectionHeader>
          <form>
            <Active :label="getI18nValue('RP.DATA.textSearch', 'Text search')">
              <TextInput v-model="search" />
            </Active>
            <Active :label="getI18nValue('RP.DATA.sortPath', 'Sort path')">
            ...
            <Active
              :label="getI18nValue('RP.DATA.resultLimit', 'Result limit')"
              :error="limit !== undefined && limit <= 0">
      ...
      <ActionBar :class="$style.actionBar">
        <PrunButton primary @click="addFilter">{{
          getI18nValue('RP.DATA.addFilter', 'ADD FILTER')
        }}</PrunButton>
        <PrunButton neutral @click="resetQuery">{{
          getI18nValue('RP.DATA.resetQuery', 'RESET QUERY')
        }}</PrunButton>
        <PrunButton primary :disabled="!result" @click="exportResult">{{
          getI18nValue('RP.DATA.downloadJson', 'DOWNLOAD JSON')
        }}</PrunButton>
      </ActionBar>

      <template v-if="filters.length > 0">
        <SectionHeader>{{ getI18nValue('RP.DATA.filters', 'Filters') }}</SectionHeader>
        <div :class="$style.filters">
          <div v-for="filter in filters" :key="filter.id" :class="$style.filterRow">
            <span :class="$style.filterLabel"
              >{{ getI18nValue('RP.DATA.filter', 'FILTER') }} {{ filter.id }}</span
            >
            <input
              v-model="filter.path"
              :class="$style.pathInput"
              :placeholder="getI18nValue('RP.DATA.pathPlaceholder', 'property.path')"
              type="text" />
            ...
            <input
              ...
              :placeholder="
                filter.operator === 'exists'
                  ? getI18nValue('RP.DATA.existsPlaceholder', 'true/false; blank = true')
                  : getI18nValue('RP.DATA.valuePlaceholder', 'JSON or text')
              "
              type="text" />
            <PrunButton danger @click="removeFilter(filter.id)">{{
              getI18nValue('RP.DATA.remove', 'REMOVE')
            }}</PrunButton>
          </div>
        </div>
      </template>
      ...
      <template v-if="source?.load">
        <SectionHeader>{{ getI18nValue('RP.DATA.explicitLoad', 'Explicit Load') }}</SectionHeader>
        <div :class="$style.loadControls">
          <input
            v-if="sourceNeedsSiteId(source)"
            v-model="siteId"
            :class="$style.siteInput"
            :placeholder="getI18nValue('RP.DATA.siteIdPlaceholder', 'Required site ID')"
            type="text" />
          <PrunButton primary :disabled="!canLoad" @click="loadSource">{{
            getI18nValue('RP.DATA.loadFromPrun', 'LOAD FROM PRUN')
          }}</PrunButton>
          <span v-if="!requestAvailable" :class="$style.error">
            {{
              getI18nValue('RP.DATA.transportUnavailable', 'PrUn request transport is unavailable.')
            }}
          </span>
          <span v-else-if="sourceNeedsSiteId(source) && !siteId.trim()" :class="$style.hint">
            {{ getI18nValue('RP.DATA.enterSiteId', 'Enter a site ID before loading.') }}
          </span>
      ...
      <details v-if="parsedParameters.connectionEnabled" :class="$style.agentPanel">
        <summary :class="$style.agentSummary">
          <span>{{ getI18nValue('RP.DATA.agentQuery', 'Agent Query Connection') }}</span>
        ...
        <div :class="$style.agentExplanation">
          {{
            getI18nValue(
              'RP.DATA.agentDesc',
              'Opt-in, authenticated, read-only, and loopback-only. The token stays in memory. Agent requests cannot load data or perform PrUn actions.',
            )
          }}
        </div>
        <form>
          <Active :label="getI18nValue('RP.DATA.loopback', 'Loopback endpoint')">
            <TextInput v-model="agentEndpoint" />
          </Active>
          <Active
            :label="getI18nValue('RP.DATA.sessionToken', 'Session token')"
            :tooltip="
              getI18nValue(
                'RP.DATA.sessionToken.tooltip',
                'Use a 32+ character shared token configured in the local agent server.',
              )
            ">
            <div :class="$style.agentToken">
              <input
                ...
                data-1p-ignore="true"
                data-lpignore="true"
                :placeholder="getI18nValue('RP.DATA.tokenPlaceholder', '32+ character token')"
                type="password" />
              <PrunButton primary inline @click="generateAgentToken">{{
                getI18nValue('RP.DATA.generate', 'GENERATE')
              }}</PrunButton>
              <PrunButton primary inline :disabled="!agentToken" @click="copyAgentToken">
                {{ getI18nValue('RP.DATA.copy', 'COPY') }}
              </PrunButton>
            </div>
          </Active>
        </form>
        <div :class="$style.commands">
          <PrunButton
            ...
            @click="connectAgent">
            {{ getI18nValue('RP.DATA.connect', 'CONNECT') }}
          </PrunButton>
          <PrunButton v-else danger inline @click="disconnectAgent">{{
            getI18nValue('RP.DATA.disconnect', 'DISCONNECT')
          }}</PrunButton>
    ...
    <div :class="$style.resultHeader">
      <SectionHeader>{{ getI18nValue('RP.DATA.result', 'Result') }}</SectionHeader>
      <span v-if="result">
        {{ result.matchedRows }} {{ getI18nValue('RP.DATA.matched', 'matched') }} /
        {{ result.totalRows }} {{ getI18nValue('RP.DATA.total', 'total') }} ·
        {{ getI18nValue('RP.DATA.showing', 'showing') }}
        {{ result.rows.length }}
      </span>
    ...
    <div v-if="result && preview.truncated" :class="$style.previewNotice">
      {{
        getI18nValue(
          'RP.DATA.previewLimited',
          'Preview limited to ${shown} rows for performance. Download includes all ${total} query rows.',
        )
          .replace('${shown}', String(preview.shownRows))
