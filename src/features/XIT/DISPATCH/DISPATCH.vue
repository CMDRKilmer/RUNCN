...
const executeTooltip = computed(() => {
  if (!hasAssignedShip.value) {
    return getI18nValue(
      'RP.DISPATCH.executeTooltip.noShip',
      'Assign a ship to at least one base first',
    );
  }
  if (overloadedShips.value.size > 0) {
    return getI18nValue(
      'RP.DISPATCH.executeTooltip.overloaded',
      'A ship is loaded above its capacity',
    );
  }
  return undefined;
});
...
<template>
  ...
  <div v-else :class="$style.layout">
    <div :class="C.ComExOrdersPanel.filter">
      ...
      <RadioItem v-model="refuel" horizontal>{{
        getI18nValue('RP.DISPATCH.refuel', 'REFUEL')
      }}</RadioItem>
      <div :class="$style.spacer" />
      <PrunButton dark @click="reset">{{ getI18nValue('RP.DISPATCH.reset', 'RESET') }}</PrunButton>
      <PrunButton
        primary
        :disabled="overloadedShips.size > 0"
        :data-tooltip="executeTooltip"
        @click="execute">
        {{ getI18nValue('RP.DISPATCH.execute', 'EXECUTE') }}
      </PrunButton>
    </div>
    <div ref="panesEl" :class="$style.panes">
      ...
      <div :class="$style.left">
        <table :class="$style.table">
          <thead>
            <tr>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.assign', 'Assign')
              }}</th>
              <GripHeaderCell />
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.planet', 'Planet')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]" colspan="2">{{
                getI18nValue('RP.DISPATCH.column.burn', 'Burn')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]" colspan="2">{{
                getI18nValue('RP.DISPATCH.column.repair', 'Rep')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.load', 'Load')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.materials', 'Materials')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.fit', 'Fit')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.days', 'Days')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.repThreshold', 'Rep ≥')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.advance', 'Adv')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.cxBuy', 'CX')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.offloadJson', 'Offload')
              }}</th>
              <th :class="[$style.narrowCol, $style.centered]">{{
                getI18nValue('RP.DISPATCH.column.agent', 'Agent')
              }}</th>
            </tr>
          </thead>
