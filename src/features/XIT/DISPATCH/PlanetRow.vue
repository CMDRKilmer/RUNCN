...
const materialFilterOptions: { label: string; value: MaterialFilter }[] = [
  { label: getI18nValue('RP.DISPATCH.materialFilter.all', 'All'), value: 'All' },
  { label: getI18nValue('RP.DISPATCH.materialFilter.workforce', 'Workforce'), value: 'Workforce' },
  {
    label: getI18nValue('RP.DISPATCH.materialFilter.production', 'Production'),
    value: 'Production',
  },
];
...
<template>
  <tr :class="$style.row">
    ...
    <td :class="$style.fitCell">
      <PrunButton dark inline :disabled="!canFit" @click="emit('fit')">{{
    ...
    <td :class="$style.advToggleCell">
      <RadioItem v-model="config.cxBuy" horizontal>{{
        getI18nValue('RP.DISPATCH.column.buy', 'BUY')
      }}</RadioItem>
    </td>
    <td :class="$style.advToggleCell">
      <RadioItem v-model="config.offloadJson" horizontal>{{
        getI18nValue('RP.DISPATCH.column.json', 'JSON')
      }}</RadioItem>
    </td>
    <td :class="$style.advToggleCell">
      <RadioItem v-model="config.agent" horizontal>{{
        getI18nValue('RP.DISPATCH.column.agt', 'AGT')
      }}</RadioItem>
    </td>
  </tr>
</template>
