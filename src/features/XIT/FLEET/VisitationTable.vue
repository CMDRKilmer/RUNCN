...
<template>
  <div v-if="rows.length === 0" :class="$style.empty">船队中暂无飞船</div>
  <table v-else :class="$style.table">
    <thead>
      <tr>
        <th rowspan="2">船载重 t</th>
        <th rowspan="2">船容积 m³</th>
        <th colspan="2">运出频率</th>
        <th colspan="2">运入频率</th>
      </tr>
      <tr>
        <th>访问间隔(天)</th>
        <th>受限维度</th>
