...
type SortKey =
  'name' | 'cargo' | 'status' | 'eta' | 'fuel' | 'none' | 'repair' | 'size' | 'shipClass';
  ...
  <div v-else :class="$style.content">
    <div :class="[C.FormComponent.containerPassive, C.forms.passive, C.forms.formComponent]">
      <label :class="[C.FormComponent.label, C.fonts.fontRegular, C.type.typeRegular]">
        {{ getI18nValue('RP.FLT.minimize', 'Minimize') }}
      </label>
    ...
    <div v-if="showFilters" :class="$style.filterPanel">
      <div :class="$style.filterMeta">
        <span v-if="activeFilterCount > 0" :class="$style.activeCount">
          {{ activeFilterCount }} {{ getI18nValue('RP.FLT.activeFilters', 'active') }}
        </span>
        <span
          v-if="activeFilterCount > 0"
          :class="[C.Link.link, $style.clearFilters]"
          @click="clearFilters">
          {{ getI18nValue('RP.FLT.resetShort', 'reset') }}
        </span>
      </div>

      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{ getI18nValue('RP.FLT.shipList', 'Ship list') }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem v-model="showStlShips" horizontal>{{
            getI18nValue('RP.FLT.stl', 'STL')
          }}</RadioItem>
          <RadioItem v-model="showFtlShips" horizontal>{{
            getI18nValue('RP.FLT.ftl', 'FTL')
          }}</RadioItem>
        </div>
      </div>

      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{
          getI18nValue('RP.FLT.flightState', 'Flight state')
        }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem v-model="showInFlightShips" horizontal>{{
            getI18nValue('RP.FLT.inFlight', 'IN FLIGHT')
          }}</RadioItem>
          <RadioItem v-model="showNotInFlightShips" horizontal>{{
            getI18nValue('RP.FLT.docked', 'DOCKED')
          }}</RadioItem>
        </div>
      </div>
      ...
      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{ getI18nValue('RP.FLT.columns', 'Columns') }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem v-model="showColName" horizontal>{{
            getI18nValue('RP.FLT.col.name', 'NAME')
          }}</RadioItem>
          <RadioItem v-model="showColShipClass" horizontal>{{
            getI18nValue('RP.FLT.col.class', 'CLASS')
          }}</RadioItem>
          <RadioItem v-model="showColSize" horizontal>{{
            getI18nValue('RP.FLT.col.size', 'SIZE')
          }}</RadioItem>
          <RadioItem v-model="showColCargo" horizontal>{{
            getI18nValue('RP.FLT.col.cargo', 'CARGO')
          }}</RadioItem>
          <RadioItem v-model="showColCargoSize" horizontal>{{
            getI18nValue('RP.FLT.col.cargoSize', 'CARGO/SIZE')
          }}</RadioItem>
          <RadioItem v-model="showColTime" horizontal>{{
            getI18nValue('RP.FLT.col.eta', 'ETA')
          }}</RadioItem>
          <RadioItem v-model="showColRepair" horizontal>{{
            getI18nValue('RP.FLT.col.repair', 'REPAIR')
          }}</RadioItem>
          <RadioItem v-model="showColFuel" horizontal>{{
            getI18nValue('RP.FLT.col.fuel', 'FUEL')
          }}</RadioItem>
          <RadioItem v-model="showColProblems" horizontal>{{
            getI18nValue('RP.FLT.col.problems', 'PROBLEMS')
          }}</RadioItem>
        </div>
      </div>

      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{ getI18nValue('RP.FLT.reset', 'Reset') }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem :model-value="false" horizontal @update:model-value="clearFilters">{{
            getI18nValue('RP.FLT.resetAllFilters', 'RESET ALL FILTERS')
          }}</RadioItem>
        </div>
      </div>
      ...
      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{
          getI18nValue('RP.FLT.primarySort', 'Primary sort')
        }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem
            v-for="key in [
      ...
      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{
          getI18nValue('RP.FLT.secondarySort', 'Secondary sort')
        }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem
            v-for="key in [
      ...
      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{ getI18nValue('RP.FLT.layout', 'Layout') }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem
            :model-value="layoutMode === 'compact'"
            horizontal
            @update:model-value="layoutMode = 'compact'"
            >{{ getI18nValue('RP.FLT.layout.compact', 'COMPACT') }}</RadioItem
          >
          <RadioItem
            :model-value="layoutMode === 'whitespace'"
            horizontal
            @update:model-value="layoutMode = 'whitespace'"
            >{{ getI18nValue('RP.FLT.layout.whitespace', 'WHITESPACE') }}</RadioItem
          >
          <RadioItem
            :model-value="layoutMode === 'cargo'"
            horizontal
            @update:model-value="layoutMode = 'cargo'"
            >{{ getI18nValue('RP.FLT.layout.cargo', 'CARGO') }}</RadioItem
          >
          <RadioItem
            :model-value="layoutMode === 'legacy'"
            horizontal
            @update:model-value="layoutMode = 'legacy'"
            >{{ getI18nValue('RP.FLT.layout.legacy', 'LEGACY') }}</RadioItem
          >
        </div>
      </div>

      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{ getI18nValue('RP.FLT.route', 'Route') }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem v-model="hideReturningToCx" horizontal>{{
            getI18nValue('RP.FLT.hideCxReturns', 'HIDE CX RETURNS')
          }}</RadioItem>
        </div>
      </div>

      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{ getI18nValue('RP.FLT.fuel', 'Fuel') }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem
      ...
      <div :class="$style.filterGroup">
        <div :class="$style.filterTitle">{{
          getI18nValue('RP.FLT.problematicFuel', 'Problematic fuel level')
        }}</div>
        <div :class="C.ComExOrdersPanel.filter">
          <RadioItem
            v-for="threshold in fuelAlertOptions"
    ...
    <component :is="tableTag" :class="layoutClass" :style="{ gridTemplateColumns }">
      ...
      <component :is="rowTag" :class="$style.headerRow">
        ...
        <component
          ...
          {{ getI18nValue('RP.FLT.status', 'Status') }}
          <span
            :class="{
