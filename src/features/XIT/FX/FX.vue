...
//   1. BUY  RAT @ HRT, currency = 源货币   （玩家付源币买 RAT → 源币出账）
//   2. SELL RAT @ HRT, currency = 目标货币（玩家卖 RAT 得目标币 → 目标币进账）
...
// 换汇方向是「源 → 目标」：玩家想把手里的源币换成目标币。
// PrUn BUY/SELL 合同里 currency 表示「合同使用的结算货币」：
//   - BUY RAT  currency=源币   → 玩家付源币 得 RAT（源币出账）；
//   - SELL RAT currency=目标币 → 玩家付 RAT 得目标币（目标币进账）。
// 合同名预设 "BUY{价格}{源币}" / "SELL{价格}{目标币}"。
...
async function handleSwap() {
  ...
  //   1. 关闭所有 CONTD 面板 → 新开一块 list → 走 helper（BUY RAT currency=源币）
  //   2. 关掉第一笔的 detail 面板 → 再开一块 list → 再走 helper（SELL RAT currency=目标币）
  //
  // 为何这么干——之前反复折戟的两个根因：
