...
//   1. SELL RAT @ HRT, currency = 目标货币（玩家卖出 RAT，目标币进账）
//   2. BUY  RAT @ HRT, currency = 源货币（玩家买入 RAT，源币出账）
...
//   - SELL RAT 收 目标币（玩家在 SELL 里收到目标币）；
//   - BUY  RAT 付 源币（玩家在 BUY 里付出源币）。
// 合同名预设 "SELL{价格}{目标币}" / "BUY{价格}{源币}"。
...
async function handleSwap() {
  ...
  //   1. 关闭所有 CONTD 面板 → 新开一块 list → 走 helper（SELL RAT 收目标币）
  //   2. 关掉第一笔的 detail 面板 → 再开一块 list → 再走 helper（BUY RAT 付源币）
  //
  // 为何这么干——之前反复折戟的两个根因：
