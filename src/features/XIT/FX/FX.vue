...
//   1. BUY  RAT @ HRT, currency = 目标货币（玩家买入 RAT，对方付目标币给玩家 → 目标币进账）
//   2. SELL RAT @ HRT, currency = 源货币（玩家卖出 RAT，对方付源币给玩家 → 源币出账）
...
// PrUn 合同里 currency 是「对方付给我的币」：
//   - BUY RAT 付 目标币 → 目标币进账（玩家在 BUY 里收到目标币）；
//   - SELL RAT 收 源币 → 源币出账（玩家在 SELL 里付出源币）。
// 合同名预设 "BUY{价格}{目标币}" / "SELL{价格}{源币}"。
...
async function handleSwap() {
  ...
  //   1. 关闭所有 CONTD 面板 → 新开一块 list → 走 helper（BUY RAT currency=目标币）
  //   2. 关掉第一笔的 detail 面板 → 再开一块 list → 再走 helper（SELL RAT currency=源币）
  //
  // 为何这么干——之前反复折戟的两个根因：
