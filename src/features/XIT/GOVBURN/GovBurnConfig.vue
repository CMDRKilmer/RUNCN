...
<template>
  <div :class="C.ComExOrdersPanel.filter">
    ...
    <PrunButton primary @click="emit('done')">{{
      getI18nValue('RP.common.done', 'DONE')
    }}</PrunButton>
  </div>

  <SectionHeader>{{ getI18nValue('RP.GOVBURN.thresholds', 'Thresholds') }}</SectionHeader>
  <form @submit.prevent>
    <Active :label="getI18nValue('RP.GOVBURN.redDays', 'Red (days)')">
      <NumberInput v-model="userData.govburn.config.red" />
    </Active>
    <Active :label="getI18nValue('RP.GOVBURN.yellowDays', 'Yellow (days)')">
      <NumberInput v-model="userData.govburn.config.yellow" />
    </Active>
  </form>

  <SectionHeader>{{ getI18nValue('RP.GOVBURN.addPlanet', 'Add Planet') }}</SectionHeader>
  <form @submit.prevent>
    <Active :label="getI18nValue('RP.GOVBURN.planet', 'Planet')" :error="planetError">
      <TextInput v-model="planetInput" @keydown="onInputKeydown" />
    </Active>
    <Commands>
      <PrunButton primary @click="addPlanet">{{ getI18nValue('RP.common.add', 'ADD') }}</PrunButton>
    </Commands>
  </form>
  <p v-if="planetError" :class="$style.error">{{
    getI18nValue('RP.GOVBURN.notFound', 'Planet not found.')
  }}</p>

  <table v-if="configuredPlanets.length > 0">
    <thead>
      <tr>
        <th>{{ getI18nValue('RP.GOVBURN.planet', 'Planet') }}</th>
    ...
    <tbody>
      <tr v-for="naturalId in configuredPlanets" :key="naturalId">
        ...
        <td>
          <PrunButton danger inline @click="removePlanet(naturalId)">{{
            getI18nValue('RP.common.remove', 'REMOVE')
          }}</PrunButton>
        </td>
      </tr>
    </tbody>
