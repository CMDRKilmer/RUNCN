...
<template>
  ...
  <table v-else>
    <thead>
      <tr>
        <th>{{ getI18nValue('RP.GOVBURN.planetView.building', 'Building') }}</th>
        <th>{{ getI18nValue('RP.GOVBURN.planetView.required', 'Required') }}</th>
        <th>{{ getI18nValue('RP.GOVBURN.planetView.days', 'Days') }}</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="row in rows" :key="row.ticker">
        ...
        <td
          ...
          :data-tooltip="
            getI18nValue('RP.GOVBURN.act.noData.tooltip', 'No data captured. Run XIT GOVBURNDATA.')
          ">
          --
        </td>
