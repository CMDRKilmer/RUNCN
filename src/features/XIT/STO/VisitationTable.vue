...
<template>
  <div v-if="rows.length === 0" :class="$style.empty">{{
    getI18nValue('RP.STO.noShips', 'No ships in fleet')
  }}</div>
  <table v-else :class="$style.table">
    <thead>
      <tr>
        <th rowspan="2">{{ getI18nValue('RP.STO.shipT', 'Ship t') }}</th>
        <th rowspan="2">{{ getI18nValue('RP.STO.shipM3', 'Ship m³') }}</th>
        <th colspan="2">{{ getI18nValue('RP.STO.exportFreq', 'Export Frequency') }}</th>
        <th colspan="2">{{ getI18nValue('RP.STO.importFreq', 'Import Frequency') }}</th>
      </tr>
      <tr>
        <th>{{ getI18nValue('RP.STO.visitationDays', 'Visitation (days)') }}</th>
        <th>{{ getI18nValue('RP.STO.col.limit', 'Limit') }}</th>
        <th>{{ getI18nValue('RP.STO.visitationDays', 'Visitation (days)') }}</th>
        <th>{{ getI18nValue('RP.STO.col.limit', 'Limit') }}</th>
      </tr>
    </thead>
